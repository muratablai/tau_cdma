...
Maps the CDMA receiver hierarchy onto particle physics ML:
  ...
  NN single-user detector     1D NN classifier + count     ~ CRB(1D)
  NN MMSE multiuser receiver  Multi-feature NN + count     → CRB(multi-D)
...
def simulate_events(theta, N, sigma_det=20.0, features='multi', seed=42):
    """Simulate individual tau decay events with realistic features.
    ...
    # PDG-informed channel properties
    props = [
        dict(mass=0.511,  width=0,     ntk=1, eid=0.90, muid=0.01, pi0=0.00),
        dict(mass=105.7,  width=0,     ntk=1, eid=0.005,muid=0.95, pi0=0.00),
        dict(mass=139.6,  width=0,     ntk=1, eid=0.02, muid=0.01, pi0=0.05),
        dict(mass=775.3,  width=149.1, ntk=2, eid=0.02, muid=0.01, pi0=0.70),
        dict(mass=1230.0, width=420.0, ntk=3, eid=0.02, muid=0.01, pi0=0.05),
        dict(mass=1100.0, width=450.0, ntk=1, eid=0.02, muid=0.01, pi0=0.70),
        dict(mass=900.0,  width=500.0, ntk=2, eid=0.02, muid=0.01, pi0=0.10),
    ]
    ...
    for k in range(K):
        ...
        p = props[k]
        if k <= 1:
            m_vis[mask] = rng.uniform(p['mass'], 1777 * 0.5, size=n_k)
        elif k == 2:
            m_vis[mask] = p['mass'] * np.ones(n_k)
        else:
            m_vis[mask] = rng.standard_cauchy(n_k) * p['width'] / 2 + p['mass']

    m_vis = np.clip(m_vis, 0, 1777)
    m_vis += rng.normal(0, sigma_det, size=N)
    m_vis = np.clip(m_vis, 0, 1800)
...
def run_ml_layer(bench, verbose=True, quick=False):
    ...
    (a) eta predicts 1D NN channel-level performance  
    (b) Multi-feature NN breaks aliasing (wideband CDMA)
    ...
    # Multi-D CRB (with PID features — theoretical floor for NN)
    pid_weight = 0.3
    ...
    emu_confusion_1d = C_1d_norm[0, 1] + C_1d_norm[1, 0]
    emu_confusion_5d = C_m_norm[0, 1] + C_m_norm[1, 0]

    if verbose:
        print(f"\n    (a) Classification performance:")
        print(f"        Overall: 1D NN = {acc_1d:.3f}, 5D NN = {acc_m:.3f}")
        print(f"        {'Ch':>6s} {'NN(1D)':>7s} {'NN(5D)':>7s} {'η_1D':>7s}"
              f" {'Interpretation':>35s}")
        for k in range(K):
            if eta_1d[k] < 0.05:
                interp = "η≈0 → 1D fails, 5D rescues"
            elif eta_1d[k] > 0.5:
                interp = "η high → both NNs succeed"
            else:
                interp = "η mid → 5D improves"
            print(f"        {labels[k]:>6s} {pca_1d[k]:>7.3f} {pca_m[k]:>7.3f} "
                  f"{eta_1d[k]:>7.4f} {interp:>35s}")

        print(f"\n        e-μ cross-confusion:")
        print(f"          1D: P(μ|e)={C_1d_norm[0,1]:.3f}, P(e|μ)={C_1d_norm[1,0]:.3f}"
              f"  (total={emu_confusion_1d:.3f})")
        print(f"          5D: P(μ|e)={C_m_norm[0,1]:.3f}, P(e|μ)={C_m_norm[1,0]:.3f}"
              f"  (total={emu_confusion_5d:.3f})")
    ...
    if verbose:
        ...
        print(f"        {'Ch':>6s} {'Bayes':>7s} {'NN(1D)':>7s} {'NN(5D)':>7s} {'Gap':>12s}")
        for k in range(K):
            gap_str = ""
            if bayes_acc[k] < 0.01:
                gap_str = "ceiling = 0%"
    ...
    if verbose:
        ...
        print(f"    │ NN single-user detector     │ 1D NN classifier           │ ~ CRB(1D)    │")
        print(f"    │ NN multiuser receiver       │ Multi-feature NN           │ → CRB(5D)    │")
        ...
        print(f"      similar templates (e/μ) interfere, inflating estimation variance.")
        print()
    ...
    # ==== Checks ====
    checks = {
        'Multi-feature NN > 1D NN accuracy': float(acc_m) > float(acc_1d) + 0.05,
        'NN breaks e-μ aliasing (5D recall > 0.8)': float(pca_m[0]) > 0.8 and float(pca_m[1]) > 0.8,
        '1D e-μ confused (cross-confusion > 0.3)': float(emu_confusion_1d) > 0.3,
        '5D resolves e-μ (cross-confusion < 0.1)': float(emu_confusion_5d) < 0.1,
        'η predicts 1D accuracy (r > 0.5)': float(eta_corr) > 0.5,
        'Bayes μ ceiling = 0% in 1D': float(bayes_acc[1]) < 0.01,
    }
